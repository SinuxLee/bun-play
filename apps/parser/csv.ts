...
function zodScalarSchema(column: ConfigColumn): string {
    switch (column.type) {
        case 'int':
...
function zodSchema(column: ConfigColumn): string {
    const scalar = zodScalarSchema(column);
    return column.isArray ? `z.array(${scalar}).readonly()` : scalar;
}
...
export function generateTypescript(table: CsvConfigTable): string {
    ...
    const zodObjectProperties = table.columns
        .filter((column) => column !== idColumn)
        .map((column) => `    ${typeScriptPropertyName(column.name)}: ${zodSchema(column)},`)
        .join('\n');
...
` : `
    ...
    constructor(rows: readonly ${identifier}WireRow[], options: ${identifier}DecodeOptions = {}) {
        for (const row of rows) {
            const id = row[${idColumn.valueIndex}];
            if (typeof id !== 'number' || this.byId.has(id)) throw new Error('invalid ${table.tableName} Id');
            if (options.validate) {
                const idResult = ${identifier}IdSchema.safeParse(id);
                if (!idResult.success) throw new Error('invalid ${table.tableName} Id ' + id + ': ' + idResult.error.message);
            }
...
${objectFromWire}
            ...
            if (options.validate) {
                const configResult = ${identifier}Schema.safeParse(config);
                if (!configResult.success) {
                    throw new Error('invalid ${table.tableName} Id ' + id + ': ' + configResult.error.message);
                }
...
/** Object shape used by gameplay code; Id remains the ${identifier}Table key. */
export const ${identifier}Schema = z.object({
${zodObjectProperties}
}).readonly();
export type ${identifier} = z.infer<typeof ${identifier}Schema>;
${idColumn === undefined ? '' : `const ${identifier}IdSchema = ${zodSchema(idColumn)};`}
export interface ${identifier}DecodeOptions {
...
export function decode${identifier}(bytes: Uint8Array, options: ${identifier}DecodeOptions = {}): ${identifier}Table {
    ...
    return new ${identifier}Table(rows as ${identifier}WireRow[], options);
}
`;
}
